...
func init() {
	var err error
	db, err = sql.Open("postgres", "postgres://postgres:@Postgres246@localhost/postgres?sslmode=disable")
	//	db, err = sql.Open("postgres", "postgres://postgres:@DatabaseUser246@localhost/postgres?sslmode=disable")
	if err != nil {
		log.Fatal(err)
	}
